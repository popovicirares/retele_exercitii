...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE: Nu esti conectat. Foloseste mai intai CONNECT.")
            continue

        argumente = parti[1] if len(parti) > 1 else ''
        if not argumente.strip():
            print("EROARE: PUBLISH necesita un mesaj nevid.")
            continue
        ...
        try:
            ...
            print("EROARE: DELETE necesita un ID numeric (numar intreg).")
            continue
