...
class FTPClient:
    ...
    def get_server_files(self):
        """Fetch files available on the server"""
        response = self.send_command({'command': 'list_files'})
        if response.get('status') != 'success':
            print(f"✗ {response.get('message', 'Unable to list files')}")
            return []
        return response.get('files', [])

    def choose_server_file(self, files, prompt='Enter file number or name: '):
        """Choose a file from a server file list"""
        if not files:
    ...
    def download(self):
        ...
        if response['status'] == 'success':
            ...
            target_name = response.get('filename', filename)
            filepath = os.path.join(LOCAL_FILES_DIR, target_name)
            try:
